...
const planCatalog = {
  starter: {
    ...
    currency: 'USD',
    summary: 'lean launch lane for one serious product',
  },
  pro: {
    id: 'pro',
    name: 'Pro',
    ...
    currency: 'USD',
    summary: 'the default route for AI-built products with real backend needs',
  },
  scale: {
    id: 'scale',
    name: 'Scale',
    ...
    currency: 'USD',
    summary: 'migration, governance, and higher-touch production help',
  },
}
...
function resolveConfiguredProductId(env, planId, billing) {
  ...
  const keys = [
    ...
    `CREEM_PRODUCT_ID_INSFORGE_${normalizedSelection}`,
...
async function getOrCreateCreemProduct(env, apiKey, plan, billing, successUrl) {
  ...
  const product = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/products`, {
    name: `InsForge ${plan.name} (${billingLabel})`,
...
async function handleCheckout(request, env, requestUrl) {
  ...
  try {
    ...
    const checkout = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/checkouts`, {
      ...
      request_id: `insforge_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      metadata: {
        site: 'insforge.space',
        planId: plan.id,
        billing,
...
function buildSitemapXml() {
  ...
  const urls = indexablePaths
    .map((path) => {
      const priority = path === '/' ? '1.0' : path === '/privacy' || path === '/terms' ? '0.4' : path === '/pricing' ? '0.9' : '0.8'
